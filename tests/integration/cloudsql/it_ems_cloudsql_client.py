...
class ItEmsCloudSqlClient(TestCase):
    ...
    def __create_table_with_dumy_values(self, table_name):
        self.CLIENT.run_sql(self.DATABASE,
    ...
    def __export_table_to_csv(self, export_uri, table_name):
        ...
        request = self.DISCOVERY_SERVICE.instances().export(project=self.GCP_PROJECT_ID,
    ...
    def __wait_for_job_done(self, ops_id):
        ops_request = self.DISCOVERY_SERVICE.operations().get(project=self.GCP_PROJECT_ID, operation=ops_id)
